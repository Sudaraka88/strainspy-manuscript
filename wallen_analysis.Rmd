...
```
sylph query /data1/gerryt/sylph_analyses/uhgg_all_c200_v0.3.0.syldb -u --read-seq-id 99.9 -t 100 -1 ./*_1.fastq.gz -2 ./*_2.fastq.gz -o ./wallen_sylph_query_uhgg_all_c200_v030.tsv

sylph profile /data1/gerryt/sylph_analyses/gtdb-r220-c200-dbv1.syldb -u --read-seq-id 99.9 -t 100 -1 ./*_1.fastq.gz -2 ./*_2.fastq.gz -o ./wallen_sylph_profile_gtdb-r220-c200-dbv1.tsv
```
...
library(tidyverse)
library(strainspy)
#TODO: THIS NEEDS TO BE RE-RUN after updates, remove all outputs and run 0724

meta_path <- "./data/wallen/wallen_metadata.csv"
meta <- readr::read_csv(meta_path) 

# Keep only OMNIgene GUT data
meta = meta[meta$collection_method != "swab",]
sylph_query_path <- "./data/wallen/wallen_sylph_query_gtdb_220_id99.tsv.gz"

se <- read_sylph(sylph_query_path)
colnames(se) <- gsub("_1", "", colnames(se))

se = se[ , which(!colnames(se) %in% c("SRR19064711", "SRR19064710", "SRR19064713", "SRR19064765", "SRR19064889", "SRR19064890") )]
...
save_path = "output_rds/wallen_logit_q_99.rds"
if(file.exists(save_path)){
  logit_fit <- readRDS(save_path)
} else {
  system.time({
    logit_fit <- caseControlFit(se,  design, nthreads = parallel::detectCores())
  })
  saveRDS(logit_fit, save_path)
}

# tophits
sort(strainspy:::clean_contig_names(top_hits(logit_fit, method = "BH")$Contig_name))
# volcano_plot
plot_volcano(logit_fit, label = T)

# Manhattan plots
plot_manhattan(logit_fit, taxonomy = taxonomy, method = "BH")
plot_manhattan(logit_fit, taxonomy = taxonomy, aggregate_by_taxa = F)

# See the top few distributions
plot_ani_dist(sy, 'Case_status', contigs = top_hits(logit_fit)$Contig_name[1:10], plot_type = "box")
plot_ani_dist(sy, 'Case_status', contigs = top_hits(logit_fit)$Contig_name[1:10], drop_zeros = T, plot_type = "box")
...
plot_manhattan(logit_fit, taxonomy = taxonomy)
plot_manhattan(logit_fit, taxonomy = taxonomy, aggregate_by_taxa = F)
```
...
save_path = "output_rds/wallen_zib_q_99.rds"
if(file.exists(save_path)){
  ZB_fit <- readRDS(save_path)
} else {
  system.time({
    ZB_fit <- glmZiBFit(sy,  design, nthreads = 4)
  })
  saveRDS(ZB_fit, save_path)
}

plot_volcano(ZB_fit, label = T)
plot_manhattan(ZB_fit, taxonomy = taxonomy)
plot_manhattan(ZB_fit, taxonomy = taxonomy, aggregate_by_taxa = F)

th <- top_hits(ZB_fit, coef = 2, alpha = 0.05, method="BH") %>%
  arrange(p_value) %>%
...
badj <- hadjust(ZB_fit, taxonomy=taxonomy)

gg <- plot_manhattan(ZB_fit, taxonomy = taxonomy, method = 'HMP')
...
plot_ani_dist(sy, 'Case_status', contigs = top_hits(ZB_fit)$Contig_name[1:100], plot_type = "box")
plot_ani_dist(sy, 'Case_status', contigs = top_hits(ZB_fit)$Contig_name[1:20], drop_zeros = T, plot_type = "box")
